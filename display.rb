...
module Interface
	def draw_tiles(x,y,z_order,content,direction=:horizontal,color=0xFFFFFFFF)
	...
	def draw_frame(x,y,width,height,z_order,type=:double,color=0xFFFFFFFF)
		case type
			when :double then
		...
		draw_tiles(x,y,z_order,tiles[:topleft],:horizontal,color)
		draw_tiles(x+width-1,y,z_order,tiles[:topright],:horizontal,color)
		draw_tiles(x+width-1,y+height-1,z_order,tiles[:bottomright],:horizontal,color)
		draw_tiles(x,y+height-1,z_order,tiles[:bottomleft],:horizontal,color)
		draw_tiles(x+1,y,z_order,[tiles[:horizontal]]*(width-2),:horizontal,color)
		draw_tiles(x+1,y+height-1,z_order,[tiles[:horizontal]]*(width-2),:horizontal,color)
		draw_tiles(x,y+1,z_order,[tiles[:vertical]]*(height-2),:vertical,color)
		draw_tiles(x+width-1,y+1,z_order,[tiles[:vertical]]*(height-2),:vertical,color)
	end
end